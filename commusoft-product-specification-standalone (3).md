...
graph TB
    subgraph External_Systems
        ...
        TITAN[Service Titan - Competitor FSM]
        ACCOUNTING[Accounting Software - Xero, QuickBooks]
        MARKETING[Marketing Tools - Mailchimp, HubSpot]
    end
    
    subgraph API_Gateway
        AUTH[Authentication - OAuth 2.0 + API Keys]
        RATE[Rate Limiter - 100 req/min per client]
        LOG[Request Logging - Audit trail]
    end
    
    subgraph API_Endpoints
        CUSTOMER_API[Customers API - GET, POST, PUT]
        JOB_API[Jobs API - GET, POST, PUT]
        TECH_API[Technicians API - GET availability]
        INVOICE_API[Invoices API - GET, POST]
        PARTS_API[Parts API - GET inventory]
    end
    
    subgraph Commusoft_Core
        DB[(Database)]
